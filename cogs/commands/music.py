...
class Music(commands.Cog):
    ...
    async def connect(self, ctx: commands.Context, *, channel: discord.VoiceChannel=None):
        ...
        if not getattr(ctx.author, "voice", None):
            nv = discord.Embed(description=f'<:error:1018174714750976030> | You are not connected to a voice channel.', color=0x2f3136)
            await ctx.send(embed=nv)
            return
        if channel is None:
            channel = ctx.author.voice.channel
        elif ctx.voice_client:
            av = discord.Embed(description=f"<:error:1018174714750976030> | I am already connected to a voice channel.", color=0x2f3136)
            await ctx.send(embed=av)
            return
        vc: wavelink.Player = await channel.connect(cls=wavelink.Player, self_deaf=True)
        sc = discord.Embed(description=f"<:GreenTick:1018174649198202990> | Successfully connected to `{channel.name}`.", color=0x2f3136)
    ...
    async def disconnect(self, ctx: commands.Context, *, channel: discord.VoiceChannel=None):
        ...
        if not getattr(ctx.author, "voice", None):
            mnv = discord.Embed(description=f"<:error:1018174714750976030> | You are not connected to a voice channel.", color=0x2f3136)
            await ctx.send(embed=mnv)
        if channel == None:
            channel = ctx.author.voice.channel
        elif not ctx.voice_client:
            mnc = discord.Embed(description=f"<:error:1018174714750976030> | I am not connected to a voice channel.", color=0x2f3136)
            await ctx.send(embed=mnc)
        elif ctx.author.voice.channel != ctx.voice_client.channel:
            svc = discord.Embed(description=f"<:error:1018174714750976030> | You should be in the same voice channel that I'm in.", color=0x2f3136)
            await ctx.send(embed=svc)
            return
        vc: wavelink.Player = ctx.voice_client
        await vc.disconnect()
        await ctx.send(embed=discord.Embed(description=f"<:GreenTick:1018174649198202990> | Successfully disconnected from `{vc.channel.name}`", color=0x2f3136))
    ...
    async def play(self, ctx: commands.Context, *, query: str):
        ...
        if not getattr(ctx.author, "voice", None):
            nvc = discord.Embed(description=f"<:error:1018174714750976030> | You are not connected to a voice channel.", color=0x2f3136)
            await ctx.send(embed=nvc)
            return
        elif not ctx.voice_client:
            vc: wavelink.Player = await ctx.author.voice.channel.connect(cls=wavelink.Player, self_deaf=True)
            svc = discord.Embed(description=f"<:GreenTick:1018174649198202990> | Successfully connected to {ctx.author.voice.channel.name}", color=0x2f3136)
            await ctx.send(embed=svc)
        else:
            vc: wavelink.Player = ctx.voice_client
    ...
    async def loop(self, ctx: commands.Context):
        if not ctx.voice_client:
            svc = discord.Embed(description=f"<:error:1018174714750976030> | I am not connected to a voice channel.", color=0x2f3136)
            await ctx.send(embed=svc)
            return
        elif not getattr(ctx.author, "voice", None):
            nvc = discord.Embed(description=f"<:error:1018174714750976030> | You are not connected to a voice channel to do this.", color=0x2f3136)
            await ctx.send(embed=nvc)
            return
        else:
            vc: wavelink.Player = ctx.voice_client
        if not vc.is_playing():
            bvc = discord.Embed(description=f"<:error:1018174714750976030> | I am not playing anything.", color=0x2f3136)
            await ctx.send(embed=bvc)
            return
        vc.loop = True if not vc.loop else False
        mvc = discord.Embed(description=f"<:GreenTick:1018174649198202990> | Looping set to {'**true**.' if vc.loop else '**false**.'}", color=0x2f3136)
    ...
    async def stop(self, ctx: commands.Context):
        if not ctx.voice_client:
            await ctx.send(embed=discord.Embed(description=f"<:error:1018174714750976030> | I am not connected to a voice channel.", color=0x2f3136))
            return
        elif not getattr(ctx.author, "voice", None):
            await ctx.send(embed=discord.Embed(description="<:error:1018174714750976030> | You are not connected to a voice channel to do this.", color=0x2f3136))
            return
        else:
            vc: wavelink.Player = ctx.voice_client
        ...
        await vc.stop()
        vc.queue.clear()
        await ctx.send(embed=discord.Embed(description=f"<:GreenTick:1018174649198202990> | Destroyed the player.", color=0x2f3136))
    ...
    async def queue(self, ctx: commands.Context):
        if ctx.invoked_subcommand is None:
            if not ctx.voice_client:
                await ctx.send(embed=discord.Embed(description="<:error:1018174714750976030> | There is nothing playing yet.", color=0x2f3136))
            ...
            if vc.queue.is_empty:
                return await ctx.send(embed=discord.Embed(description="<:error:1018174714750976030> | There are no more tracks in the queue.", color=0x2f3136))
    ...
    async def clear(self, ctx: commands.Context):
        if not ctx.voice_client:
            await ctx.send(embed=discord.Embed(description="<:error:1018174714750976030> | I am not connected to a voice channel.", color=0x2f3136))
            return
        elif not getattr(ctx.author, "voice", None):
            await ctx.send(embed=discord.Embed(description="<:error:1018174714750976030> | You are not connected to a voice channel to do this.", color=0x2f3136))
            return
    ...
    async def pause(self, ctx: commands.Context):
        ...
        if not vc.is_playing():
            return await ctx.send(embed=discord.Embed(description="<:error:1018174714750976030> | There is nothing playing yet.", color=0x2f3136))
        await vc.pause()
    ...
    async def resume(self, ctx: commands.Context):
        ...
        if not vc.is_playing():
            return await ctx.send(embed=discord.Embed(description="<:error:1018174714750976030> | There is nothing playing yet.", color=0x2f3136))
        await vc.resume()
        await ctx.send(embed=discord.Embed(description="<:GreenTick:1018174649198202990> | Resumed the player.", color=0x2f3136))
    ...
    async def skip(self, ctx: commands.Context):
        ...
        await vc.play(track)
        await ctx.send(embed=discord.Embed(description="<:GreenTick:1018174649198202990> | Skiped the track.", color=0x2f3136))
        await ctx.send(embed=discord.Embed(title="Now Playing", description=f"{track.title} [{ctx.author.mention}]", color=0x2f3136))
