...
<:mod_untick:1074674904219258950> You need certain Quantun-SPEX Permissions to use this command!
    ...
    async def _ban(self,
        ...
        if ctx.author == ctx.guild.owner or str(ctx.author.id) in admin or adminrole in ctx.author.roles:
            if member == self.bot:
                ...
                color=0x00FFE4,
                description=
                f"<:GreenTick:1029990379623292938> | {member.display_name} has been successfully banned")
                    hacker.set_author(name=f"{ctx.author.name}",
                              icon_url=f"{ctx.author.avatar}")
...
<:mod_untick:1074674904219258950> You need certain Quantun-SPEX Permissions to use this command!
    ...
    async def _lock(self,
        ...
        try:
            ...
            await ctx.send(
                embed=discord.Embed(title="Quantun-SPEX | Lockdown",
    ...
    async def _unlock(self,
        ...
        try:
            ...
            await ctx.send(
                embed=discord.Embed(title="Quantun-SPEX | Unlockdown",
...
**Description:** {pack.description}
    ...
    async def humans(self,ctx,role:discord.Role):
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            hacker1 = discord.Embed(
                description=f"<:GreenTick:1018174649198202990> | Added {role.mention} to all humans .",
                color=0x2f3136)
    ...
    async def tokens(self,ctx,role:discord.Role):
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            hacker1 = discord.Embed(
                description=f"<:GreenTick:1018174649198202990> | Added {role.mention} to all token members .",
                color=0x2f3136)
            hacker1.set_author(name=f"{ctx.author}",
                               icon_url=f"{ctx.author.avatar}")
    ...
    async def bots(self,ctx,role:discord.Role):
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            hacker1 = discord.Embed(
                description=f"<:GreenTick:1018174649198202990> | Added {role.mention} to all bots .",
                color=0x2f3136)
            hacker1.set_author(name=f"{ctx.author}",
                               icon_url=f"{ctx.author.avatar}")
            for hacker in ctx.guild.members:
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if len(wled) == 15:
                hacker = discord.Embed(
                    title="Quantun-SPEX",
                    description=
                    "<:mod_arrow:1074674871759540345> You are trying to add more users than what it's allowed in [Trusted Admins]!",           color=0x2f3136)
                hacker.set_author(
                ...
                if str(user.id) in wled:
                    ...
                    value=f"""<:mod_arrow:1074674871759540345> `{user}` <:mod_untick:1074674904219258950>
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if str(user.id) in wled:
                ...
                hacker = discord.Embed(
                    color=0x2f3136,
                    title="Quantun-SPEX",
                    description=f"<:GreenTick:1029990379623292938> | Successfully Removed {user.mention} From Admin list For {ctx.guild.name}"
                )
                await ctx.reply(embed=hacker, mention_author=False)
            else:
