...
class Extra(commands.Cog):
    ...
    async def stats(self, ctx):
        """Shows some usefull information about Quantun-SPEX"""
        ...
        embed = discord.Embed(
            color=0x2f3136,
            description=
            "[Invite](https://discord.com/oauth2/authorize?client_id=1017817341582659655&permissions=2113268958&scope=bot) ● [Support Server](https://discord.gg/mYpyeBVwB4)"
        )

        embed.add_field(
            name=
            '<a:hacker_bannerstory_discord:1063109545825673356> • **Servers**',
            value=f'```Total: {serverCount} Server```')
        embed.add_field(name='<:hacker_Users:1063109685726683146> • **Users**',
                        value=f'```Total: {total_members} Users```')
        embed.add_field(
            name="<:hacker_cpu:1063109733264920596> • **System**",
            value=
            f"```RAM: {used_memory}/{total_memory} MB\nCPU: {cpu_used}% used.```"
        )
        embed.add_field(
            name="<:python:1063109575676543016> • **Python Version**",
            value=f"```{sys.version}```"),
        embed.add_field(
            name='<:python:1063109575676543016> • **Discord.py Version**',
            value=f'```{discord.__version__}```')
        embed.add_field(
            name="<a:hacker_botping:1063109767045845132> • **Ping**",
        ...
        embed.add_field(
            name=
            '<a:hacker_developer_bot:1063109615321100368> • **Developers**',
            value=
            f"[{hacker}](https://discord.com/users/{hacker.id})\n[{luci}](https://discord.com/users/{luci.id})\n[{hasan}](https://discord.com/users/301502732664307716)"
        )

        embed.set_author(name=f"{self.bot.user.name} Stats",
                         icon_url=self.bot.user.display_avatar.url)
        embed.set_thumbnail(url=self.bot.user.display_avatar.url)
        embed.set_footer(text='Thanks For Using Quantun-SPEX',
                         icon_url=self.bot.user.display_avatar.url)
    ...
    async def invite(self, ctx: commands.Context):
        embed = discord.Embed(
            description=
            "> • [Click Here To Invite Quantun-SPEX To Your Server](https://discord.com/oauth2/authorize?client_id=931816694786170890&permissions=2113268958&scope=bot)\n> • [Click Here To Join My Support Server](https://discord.gg/mYpyeBVwB4)",
            color=0x2f3136)
    ...
    async def botinfo(self, ctx: commands.Context):
        ...
        embed = discord.Embed(color=0x2f3136,
                              title="Quantun-SPEX's Information",
                              description=f"""
...
**Color:** {member.color if member.color else '000000'}
    ...
    async def removeemoji(self, ctx, emoji: discord.Emoji):
        await emoji.delete()
        await ctx.send(
            "**<a:black_Xenon:1002204507985432666> emoji has been deleted.**"
        )
    ...
    async def ping(self, ctx):
        embed = discord.Embed(
            title=
            """<a:hacker_botping:1063109767045845132> Ping <a:hacker_botping:1063109767045845132>""",
    ...
    async def _badges(self, ctx, user: Optional[discord.User] = None):
        ...
        if hacker in sup.members:
            ...
            if han in hacker.roles:
                bdg +="<a:NY_Admin:1068817878226309120>・**Admin**\n"
            if mod in hacker.roles:
                bdg +="<:modrator:1068817956945023006>・**Mod**\n"
            if staff in hacker.roles:
                bdg +="<:astroz_admin:1068819315396182048>・**Staff**\n"
